...
def run_trainer(config):
    ...
    run_name = run_name_format.format(
        **config, timestamp=datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    )
    ...
    source_dictionary = IndexDictionary.load(
        config["data_dir"], mode="source", vocabulary_size=config["vocabulary_size"]
    )
    target_dictionary = IndexDictionary.load(
        config["data_dir"], mode="target", vocabulary_size=config["vocabulary_size"]
    )
    ...
    model = build_model(
        config, source_dictionary.vocabulary_size, target_dictionary.vocabulary_size
    )

    logger.info(model)
    logger.info(
        "Encoder : {parameters_count} parameters".format(
            parameters_count=sum([p.nelement() for p in model.encoder.parameters()])
        )
    )
    logger.info(
        "Decoder : {parameters_count} parameters".format(
            parameters_count=sum([p.nelement() for p in model.decoder.parameters()])
        )
    )
    logger.info(
        "Total : {parameters_count} parameters".format(
    ...
    val_dataloader = DataLoader(
        val_dataset, batch_size=config["batch_size"], collate_fn=input_target_collate_fn
    )
